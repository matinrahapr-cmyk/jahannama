...
const img = (name: string) => withBasePath(`/images/products/${name}.webp`);
...
export const storeProducts: StoreProduct[] = [
  ...
  {name:"آجر نسوز سفید ۱۰×۶۰",code:"JN-F03",category:"facade",subcategory:"white-brick",color:"سفید",size:"۱۰ × ۶۰",status:"موجود",image:img("pa-m04")},
  {name:"آجر نسوز طوسی ۲۰×۸۰",code:"JN-F04",category:"facade",subcategory:"gray-brick",color:"طوسی",size:"۲۰ × ۸۰",status:"سفارشی",image:img("pa-t08")},
  {name:"آجر نسوز قهوه‌ای ۱۲×۱۰۰",code:"JN-F05",category:"facade",subcategory:"brown-brick",color:"قهوه‌ای",size:"۱۲ × ۱۰۰",status:"سفارشی",image:img("pa-i03")},
  {name:"آجر نسوز مشکی",code:"JN-F06",category:"facade",subcategory:"black-brick",color:"مشکی",size:"۷ × ۳۱",status:"موجود",image:img("pa-m05")},
...
export function productHref(code: string) {
  return withBasePath(`/products/${code.toLowerCase()}/`);
}

export function catalogHref(slug: string) {
  const code = catalogProductCodes[slug];
  return code ? productHref(code) : `${withBasePath("/")}?category=${slug}#store-products`;
}
