...
export type Product = { name:string; code:string; color:string; meta:string; price:string; badge?:string; image:string };
export function ProductCard({product, index}:{product:Product; index:number}) {
  return <article className="productCard">
    <div className={`productVisual productVisual${index+1}`}><span className="productBadge">{product.badge ?? "پرفروش"}</span><img src={product.image} alt={`نمای نزدیک ${product.name}`} loading="lazy"/></div>
