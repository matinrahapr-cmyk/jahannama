  ...
  return <>
    ...
    <header className="header">
      <div className="container headerInner">
        ...
        <div className="headerActions"><button className="iconBtn" aria-label="جستجو"><Search/></button><a className="phoneLink" href="tel:+989133678359"><Phone/> ۰۹۱۳۳۶۷۸۳۵۹</a><a className="btn btnPrimary btnSmall" href="/#quote">دریافت قیمت</a><button className="mobileMenu" aria-expanded={open} aria-label={open ? "بستن منو" : "بازکردن منو"} onClick={() => setOpen(!open)}><Menu/></button></div>
      </div>
      <div className={`mobileDrawer ${open ? "isOpen" : ""}`}><div className="mobileDrawerInner"><strong className="mobileProductsTitle">محصولات</strong>{catalog.map(node => <MenuBranch key={node.slug} node={node} onNavigate={closeMenus}/>)}<a href="/#projects" onClick={closeMenus}>پروژه‌ها</a><a href="/#calculator" onClick={closeMenus}>محاسبه‌گر آجر</a><a href="/#contact" onClick={closeMenus}>تماس با ما</a></div></div>
    </header>
  </>;
}
