...
export default async function ProductPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  if (!product) {
    return <main><Header/><section className="section"><div className="container emptyProduct"><h1>محصول پیدا نشد</h1><a className="btn btnPrimary" href={sitePath("/#store-products")}>بازگشت به محصولات</a></div></section></main>;
  }
    ...
    <section className="productDetailSection">
      <div className="container">
        <nav className="productBreadcrumb" aria-label="مسیر صفحه">
          <a href={sitePath("/")}>صفحه اصلی</a><span>←</span><a href={sitePath("/#store-products")}>محصولات</a><span>←</span><span>{product.name}</span>
        </nav>
        <div className="productDetailGrid">
          ...
          <article className="productDetailInfo">
            ...
            <div className="detailActions">
              <a className="btn btnPrimary" href={sitePath("/#quote")}>استعلام قیمت این محصول <ArrowLeft/></a>
        ...
        <div className="backToProducts"><a className="textLink" href={sitePath("/#store-products")}>بازگشت به همه محصولات <ArrowLeft/></a></div>
      </div>
    </section>
  </main>;
}
